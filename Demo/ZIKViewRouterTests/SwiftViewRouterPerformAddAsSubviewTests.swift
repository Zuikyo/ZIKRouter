//
//  SwiftViewRouterPerformAddAsSubviewTests.swift
//  ZIKViewRouterTests
//
...
class SwiftViewRouterPerformAddAsSubviewTests: ZIKViewRouterTestCase {
    
    weak var testRouter: DestinationViewRouter<BSwiftSubviewInput>? {
        willSet {
            self.router = newValue?.router
            self.strongTestRouter = newValue
        }
    }
    var strongTestRouter: DestinationViewRouter<BSwiftSubviewInput>?
    ...
    func testPerformWithPrepareDestination() {
        ...
        enterTest { (source) in
            self.testRouter = Router.perform(
                to: RoutableView<BSwiftSubviewInput>(),
                ...
                configuring: { (config, prepareDest, _) in
                    ...
                    config.successHandler = { destination in
                        XCTAssert(destination is BSwiftSubviewInput)
                        XCTAssert((destination as! BSwiftSubviewInput).title == "test title")
                        self.handle({
    ...
    func testPerformOnDestinationSuccess() {
        ...
        enterTest { (source) in
            let destination = Router.makeDestination(to: RoutableView<BSwiftSubviewInput>())
