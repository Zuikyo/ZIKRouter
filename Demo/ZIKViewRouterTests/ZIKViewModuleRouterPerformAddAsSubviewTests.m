...
- (void)testPerformWithSuccessCompletion {
    ...
    {
        [self enterTest:^(UIViewController *source) {
            self.router = [ZIKRouterToViewModule(BSubviewModuleInput) performPath:[self pathFromSource:source] completion:^(BOOL success, id<BSubviewInput>  _Nullable destination, ZIKRouteAction  _Nonnull routeAction, NSError * _Nullable error) {
                XCTAssertTrue(success);
                XCTAssertNil(error);
                [expectation fulfill];
                [self handle:^{
