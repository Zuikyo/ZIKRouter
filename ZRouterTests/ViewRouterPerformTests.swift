...
class ViewRouterPerformTests: XCTestCase {
    ...
    func testPerformWithPreparation() {
        ...
        enterTest()
        self.router = Router.perform(to: RoutableView<AViewInput>(), path: .makeDestination, preparation: { (destination) in
            expectation.fulfill()
            self.handle({
                self.leaveTest()
            })
    ...
    func testPerformWithSuccessHandler() {
        ...
        enterTest()
        self.router = Router.perform(to: RoutableView<AViewInput>(), path: .makeDestination, successHandler: { (destination) in
            expectation.fulfill()
            self.handle({
                self.leaveTest()
            })
    ...
    func testPerformWithErrorHandler() {
        ...
        TestConfig.routeShouldFail = true
        enterTest()
        self.router = Router.perform(to: RoutableView<AViewInput>(), path: .makeDestination, errorHandler: { (action, error) in
            expectation.fulfill()
            self.handle({
                self.leaveTest()
            })
