...
class ServiceRouterPerformTests: XCTestCase {
    ...
    func testPerformWithPrepareDestination() {
        ...
        enterTest()
        self.router = Router.perform(to: RoutableService<AServiceInput>(), configuring: { (config, prepareModule) in
    ...
    func testPerformWithSuccessCompletion() {
        let expectation = self.expectation(description: "completionHandler")
        enterTest()
        self.router = Router.perform(to: RoutableService<AServiceInput>(), completion: { (success, destination, action, error) in
            XCTAssertTrue(success)
            XCTAssertNotNil(destination)
    ...
    func testPerformWithSuccessHandler() {
        ...
        enterTest()
        self.router = Router.perform(to: RoutableService<AServiceInput>(), successHandler: { (destination) in
            expectation.fulfill()
            self.handle({
                self.leaveTest()
            })
    ...
    func testPerformWithErrorHandler() {
        ...
        TestConfig.routeShouldFail = true
        enterTest()
        self.router = Router.perform(to: RoutableService<AServiceInput>(), errorHandler: { (action, error) in
            expectation.fulfill()
            self.handle({
                self.leaveTest()
            })
