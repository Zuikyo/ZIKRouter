...
class ViewModuleRouterPerformTests: XCTestCase {
    ...
    func testPerformWithPrepareDestination() {
        ...
        enterTest()
        self.router = Router.perform(to: RoutableViewModule<AViewModuleInput>(), path: .makeDestination, configuring: { (config, prepareModule) in
    ...
    func testPerformWithSuccessCompletion() {
        let expectation = self.expectation(description: "completionHandler")
        enterTest()
        self.router = Router.perform(to: RoutableViewModule<AViewModuleInput>(), path: .makeDestination, completion: { (success, destination, action, error) in
            XCTAssertTrue(success)
            XCTAssertNotNil(destination)
            XCTAssert(destination is AViewInput)
            expectation.fulfill()
    ...
    func testPerformWithPreparation() {
        ...
        enterTest()
        self.router = Router.perform(to: RoutableViewModule<AViewModuleInput>(), path: .makeDestination, preparation: { (module) in
            expectation.fulfill()
            self.handle({
                self.leaveTest()
            })
