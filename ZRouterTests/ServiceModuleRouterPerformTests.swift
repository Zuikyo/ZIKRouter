//
//  ServiceModuleRouterPerformTests.swift
//  ZRouterTests
//
...
class ServiceModuleRouterPerformTests: XCTestCase {
    ...
    func testPerformWithPrepareDestination() {
        ...
        enterTest()
        self.router = Router.perform(to: RoutableServiceModule<AServiceModuleInput>(), configuring: { (config, prepareDestiantion, prepareModule) in
    ...
    func testPerformWithSuccessCompletionHandler() {
        ...
        enterTest()
        self.router = Router.perform(to: RoutableServiceModule<AServiceModuleInput>(), configuring: { (config, prepareDestiantion, _) in
    ...
    func testPerformWithSuccessCompletion() {
        let expectation = self.expectation(description: "completionHandler")
        enterTest()
        self.router = Router.perform(to: RoutableServiceModule<AServiceModuleInput>(), completion: { (success, destination, action, error) in
            XCTAssertTrue(success)
            XCTAssertNotNil(destination)
            XCTAssert(destination is AServiceInput)
            expectation.fulfill()
    ...
    func testPerformWithSuccess() {
        ...
        self.router = Router.perform(to: RoutableServiceModule<AServiceModuleInput>(), configuring: { (config, prepareDestination, prepareModule) in
            ...
            config.successHandler = { d in
                let destination = d as? AServiceInput
                XCTAssertNotNil(destination)
